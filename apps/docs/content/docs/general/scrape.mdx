...
| `wait_for_selector` | string or object | No       | -              | Wait for a specific selector (browser engines only). String form is CSS selector; object form: `{ selector: string, state?: "attached" | "visible" | "hidden" | "detached", timeout?: number }`. Takes priority over `wait_for`. |
